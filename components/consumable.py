...
from __future__ import (
    annotations,
)  # Permite la anotación de tipos para clases que se definen después en el código.
from typing import (
    Optional,
    TYPE_CHECKING,
    Callable,
)  # Importa herramientas para el manejo de tipos en el código.
from components.base_component import (
    BaseComponent,
)  # Importa la clase base para los componentes de los ítems.
from core.exceptions import (
    Impossible,
)  # Importa la excepción personalizada "Impossible".
from entities.entity import (
    Actor,
)  # Importa la clase Actor, que representa a los personajes en el juego.

import systems.actions as actions  # Importa las acciones que los actores pueden realizar.
from ui import (
    colors as color,
)  # Importa los colores utilizados en la interfaz gráfica o mensajes.
...
import core.exceptions as exceptions  # Importa las excepciones personalizadas.

from ui.input_handlers import (  # Importa los manejadores de entrada de acciones y ataques.
...
if TYPE_CHECKING:
    from entities.entity import (
        Actor,
        Item,
    )  # Importa las clases Actor e Item solo para la comprobación de tipos.
    ...
    def get_action(self, consumer: Actor) -> SingleRangedAttackHandler:
        ...
        return SingleRangedAttackHandler(
            ...
            callback=lambda xy: actions.ItemAction(
                consumer, self.parent, xy
            ),  # Acción que se realiza al elegir un objetivo.
        )
...
class FireballDamageConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        targets_hit = (
            False  # Bandera que indica si se ha golpeado al menos a un objetivo.
        )
...
class HealingConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        if amount_recovered > 0:
            self.engine.message_log.add_message(
                f"Recuperaste {amount_recovered} puntos de vida.",
...
class LightningDamageConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        closest_distance = (
            self.maximum_range + 1.0
        )  # Establece una distancia máxima mayor que el rango.
        ...
        if target:
            ...
            raise Impossible(
                "Ningun enemigo a la vista para atacar."
            )  # Si no hay enemigo, lanza un error.
...
class DefensiveScrollConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        consumer = (
            action.entity
        )  # Aseguramos que consumer sea el actor que usa el ítem.
        ...
        consumer.fighter.activate_defense_bonus(
            self.defense_bonus, self.number_of_turns
        )
        self.consume()
...
class InvisibilityScrollConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        consumer.invisibility_turns = (
            self.number_of_turns
        )  # Establece los turnos de invisibilidad.
        self.engine.message_log.add_message(
