...
from __future__ import (
    annotations,
)  # Asegura compatibilidad con anotaciones de tipo futuras.
...
from core.engine import Engine  # La clase principal para el motor del juego.
from core.game_map import GameWorld  # La clase que define el mundo del juego.
...
from ui import colors as color  # Módulo personalizado con colores para mensajes.
import entities.factories as entity_factories  # Factores para crear entidades (jugador, objetos, etc.).
from ui import input_handlers  # Gestiona las entradas del usuario.
import core.exceptions as exceptions  # Maneja excepciones personalizadas.
...
def new_game(context: tcod.context.Context, console: tcod.Console) -> Engine:
    ...
    engine = Engine(
        player=player, context=context, console=console
...
def load_game(
    filename: str, context: tcod.context.Context, console: tcod.Console
) -> Engine:
    """Carga una instancia de Engine desde un archivo y restaura el contexto y la consola."""
    with open(filename, "rb") as f:
        engine = pickle.loads(
            lzma.decompress(f.read())
        )  # Descomprime y carga el objeto.
    assert isinstance(
        engine, Engine
    )  # Asegura que el objeto cargado es una instancia de Engine.
...
def get_player_name(
    ...
    prompt_x = (
        screen_width - len(prompt_text)
    ) // 2  # Posición horizontal de la solicitud.
    prompt_y = screen_height // 3  # Posición vertical de la solicitud.
    name_x = (
        screen_width // 2 - 10
    )  # Posición horizontal del cuadro de texto para el nombre.
    ...
    while True:  # Bucle para capturar la entrada del nombre.
        ...
        if len(name) < name_width:
            display_name = (
                display_name[: len(name)] + cursor + display_name[len(name) + 1 :]
            )
        ...
        for event in tcod.event.get():
            if isinstance(event, tcod.event.Quit):  # Si el jugador sale del juego.
                ...
                ):  # Si se presiona espacio, agrega un espacio al nombre.
                    if len(name) < name_width:
                        name += " "
                elif (
                    event.sym == tcod.event.KeySym.ESCAPE
                ):  # Si presiona Escape, cierra el juego.
                    raise SystemExit()
                elif (
                    len(event.sym.name) == 1 and event.sym.name.isalpha()
                ):  # Si el carácter es una letra, lo agrega al nombre.
                    if len(name) < name_width:
...
class MainMenu(input_handlers.BaseEventHandler):
    ...
    def __init__(self, context: tcod.context.Context, console: tcod.Console):
        ...
        self.last_player_name = (
            None  # Almacena el nombre del jugador de la última partida
        )
        ...
        if event.sym in (
            ...
            try:
                return input_handlers.MainGameEventHandler(
                    load_game("savegame.sav", self.context, self.console)
                )
            except FileNotFoundError:
                return input_handlers.PopupMessage(
                    self, "No hay una anterior partida guardada."
                )
            ...
            handler = input_handlers.MainGameEventHandler(
                engine
            )  # Prepara el handler del juego.
            ...
            while True:
                ...
                for i, (fg, text) in enumerate(legend_text):
                    self.console.print(
                        x=frame_x + 1, y=frame_y + 1 + i, string=text, fg=fg
                    )

                self.context.present(
                    self.console
                )  # Muestra el contenido de la consola.
