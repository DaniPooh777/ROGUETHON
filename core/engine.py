...
from __future__ import (
    annotations,
)  # Permite la anotación de tipos con clases que aún no están definidas.
from typing import (
    TYPE_CHECKING,
    Callable,
)  # Importa TYPE_CHECKING para la comprobación de tipos en tiempo de desarrollo.
from tcod.console import (
    Console,
)  # Importa la clase Console de la biblioteca tcod para la salida gráfica.
from tcod.map import (
    compute_fov,
)  # Importa compute_fov para calcular el campo de visión (FOV).
from components.base_component import (
    BaseComponent,
)  # Importa la clase base para componentes.
from ui.message_log import MessageLog  # Importa el sistema de registro de mensajes.
...
import systems.rendering as render_functions  # Importa funciones de renderizado personalizadas.
...
if TYPE_CHECKING:
    from entities.entity import (
        Actor,
    )  # Importa la clase Actor (para el jugador y enemigos).
    from core.game_map import (
        GameMap,
        GameWorld,
    )  # Importa las clases GameMap y GameWorld.
...
class Engine:
    ...
    def __init__(
        self, player: Actor, context: tcod.context.Context, console: tcod.Console
    ):
        """Inicializa el motor del juego con el jugador, contexto y consola."""
        self.message_log = (
            MessageLog()
        )  # Crea un objeto para registrar los mensajes del juego.
    ...
    def process_scheduled_tasks(self) -> None:
        ...
        for task in list(self.scheduled_tasks):  # Itera sobre las tareas programadas.
            task_turn, callback = (
                task  # Desempaqueta el turno y la función de la tarea.
            )
    ...
    def update_fov(self) -> None:
        ...
        self.game_map.explored |= (
            self.game_map.visible
        )  # Marca como explorado lo visible.
    ...
    def save_as(self, filename: str) -> None:
        ...
        try:
            save_data = lzma.compress(
                pickle.dumps(self)
            )  # Serializa y comprime el estado del motor.
            with open(
                filename, "wb"
            ) as f:  # Abre el archivo en modo escritura binaria.
    ...
    def handle_events(self, events: list[tcod.event.Event]) -> None:
        ...
        for event in events:  # Itera sobre los eventos.
            ...
            ):  # Verifica si el evento es de tipo Quit.
